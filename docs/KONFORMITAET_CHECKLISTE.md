...
Frage: Tragen die in Zeile 9 genannten Belege die Anforderung, die Kap. 3.4 „Analyse der
...
im Kapitel 3.4 stimmen PDF-Seitenzahl und gedruckte Seitenzahl überein.

**Fortschreibung 25.09.2026 (T-0946-cto, Abschluss des Vorhabens T-0870-ceo).** Die erste Gegenprobe
(T-0741-ceo, 23.09.2026) urteilte 1 × trägt, 7 × teilweise, 5 × trägt nicht. Seitdem haben die Teilpakete
T-0933-cto bis T-0945-cto den Bestand erweitert. Die Anforderungen A1 bis A13 und ihre Seiten sind
unverändert aus der ersten Gegenprobe übernommen; neu beurteilt ist der Bestand. Dafür wurden die neun
Belegdateien der Zeile 9 vollständig gelesen und der Dienst `querverbindungs_auswertung()` einmal
ausgeführt. Ergebnis: 52 Klimawirkungen im Katalog, davon 14 mit Netzrolle und 16 mit benannter
Einzelbeziehung; 13 Netzknoten der Gesamtbetrachtung stehen nicht im Katalog (#1, #14, #15, #26, #38,
#40, #43, #48, #49, #73, #92, #97, #101) und werden als `netzknoten_ausserhalb_katalog` mit dem Hinweis
„nicht im Katalog“ ausgeliefert. Dazu kommen 3 Rückkopplungen (RK-1 bis RK-3), 6 Befunde der Auswertung
der hoch bewerteten Klimawirkungen und 3 wörtliche Aussagen (`annahme`, `einordnung_cluster`,
`kaskadeneffekte`). `NETZROLLEN` führt 27 Klimawirkungen: 25 aus der Gesamtbetrachtung, #7 und #8 nur
aus der Auswertung der hoch bewerteten Klimawirkungen. `BENANNTE_BEZIEHUNGEN` führt 27 Einträge.

| Nr | Anforderung (Wortlaut oder enge Wiedergabe) | Seite | tragender Beleg (Datei, Funktion oder Abschnitt) | Urteil |
|---|---|---|---|---|
| A1 | Querbezüge zwischen den einzelnen Klimawirkungen (102 Klimawirkungen, 13 Handlungsfelder) werden identifiziert, „basierend auf den in den Kapiteln der Handlungsfelder dargestellten Zusammenhängen“; eingeschlossen sind die vorgelagerten Klimawirkungen, die in den Wirkungsketten direkt den klimatischen Einflüssen folgen (Fn. 20). | 82 | `backend/app/data/kwra_querverbindungen.py`, `BENANNTE_BEZIEHUNGEN` (27 Einträge) und `NETZROLLEN` (27 Klimawirkungen); `backend/app/services/querverbindungen.py`, `querverbindungs_auswertung()`; `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`, Abschnitte „Benannte Einzelbeziehungen“ und „Modellgrenze“ | trägt teilweise |
| A2 | Zusätzlich wird „ein Abgleich mit den dargestellten Querverbindungen und Wirkbeziehungen in den Klimawirkungsketten durchgeführt (UBA 2016)“. | 82 | `docs/QUERVERBINDUNGEN_ABGLEICH_UBA2016.md` (15 Beziehungen: 7 × bestätigt, 6 × teilweise, 2 × nicht dargestellt), verlinkt aus `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`, Abschnitt „Benannte Einzelbeziehungen“ | trägt |
| A3 | Wirkbeziehungen werden gerichtet erfasst (von einer Klimawirkung zu einer nachgelagerten; aus- und eingehend, „vor- und nachgelagerte Klimawirkungen“); davon werden gegenseitige Wechselwirkungen unterschieden. | 82, 88 | `kwra_querverbindungen.py`, Feld `richtung` („gerichtet“, „gegenseitig“) in `BENANNTE_BEZIEHUNGEN`; `querverbindungen.py`, Zähler `ausgehende_benannte`/`eingehende_benannte` (gegenseitig bei beiden Enden in beide Richtungen); `RiskInteractionSection.tsx`, Spalten „wirkt auf (benannt)“ und „beeinflusst von (benannt)“ | trägt |
| A4 | Die Auswertung „basiert auf der Annahme, dass negative Auswirkungen des Klimawandels auf eine Klimawirkung auch negative Folgen für die ihr nachgelagerten Wirkungen haben“. | 82 | `kwra_querverbindungen.py`, `AUSSAGEN["annahme"]`; `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`, Abschnitt „Annahme der Auswertung und Einordnung“; `QuerverbindungenHandlungsfelder.tsx`, Lesehinweis mit Fundstelle | trägt |
| A5 | Ausgewertet wird, „welche Klimawirkungen und aggregiert auch Handlungsfelder sich auf besonders viele andere … auswirken oder welche andersherum von besonders vielen beeinflusst werden“ (Ergebnisse S. 84–85: meiste ausgehende Wirkungen „Wasserhaushalt, Wasserwirtschaft“, meiste eingehende „Tourismuswirtschaft“; Kernaussage: „Schäden an touristischen Infrastrukturen und Betriebsunterbrechungen“ wird von den meisten anderen beeinflusst). | 82, 84–85, 88 | `kwra_querverbindungen.py`, `NETZROLLEN` und `KENNZAHLEN`; `kwra_handlungsfeld_querbezuege.py`, `HANDLUNGSFELDER`; `querverbindungen.py`, Felder `netzrollen`, `netzknoten_ausserhalb_katalog`, `handlungsfelder`; `RiskInteractionSection.tsx` (alle Netzknoten, auch #92 mit „nicht im Katalog“); `QuerverbindungenHandlungsfelder.tsx` (Rangfolge der 13 Handlungsfelder, Kennzahlen) | trägt |
| A6 | „Klimawirkungen mit zentraler Bedeutung (also einer hohen Anzahl an ausgehenden Wirkungen)“ werden identifiziert: „Hochwasser“ wirkt sich auf die meisten anderen Klimawirkungen aus; unter den hoch bewerteten Klimawirkungen nimmt „Verschiebung von Arealen und Rückgang der Bestände“ eine zentrale Position ein. | 82, 84, 87, 88 | `kwra_querverbindungen.py`, `NETZROLLEN`, Feld `zentral` (#49 und #4, mit `beleg_rolle`); `querverbindungen.py`, Feld `zentral` je Klimawirkung und je Netzknoten außerhalb des Katalogs; `RiskInteractionSection.tsx`, Netzrolle mit Zusatz „zentral“ | trägt |
| A7 | Kennzahlen des Netzes: 257 Querverbindungen, im Durchschnitt circa 2,5 ein- oder ausgehende Wirkungen je Klimawirkung, 71 Klimawirkungen mit ausgehenden und 62 mit eingehenden Wirkungen. | 83–84, 88 | `kwra_querverbindungen.py`, `KENNZAHLEN`; ausgeliefert über `GET /catalog/querverbindungen`; `RiskInteractionSection.tsx` zeigt 257 und den Durchschnitt, `QuerverbindungenHandlungsfelder.tsx` „71 von 102“ und „62 von 102“ | trägt |
| A8 | Eine Klimawirkung kann zugleich ausgehende und eingehende Wirkbeziehungen haben (Fn. 21); „Verschiebung von Arealen und Rückgang der Bestände“ hat „viele ausgehende und eingehende Wirkbeziehungen“. | 84, 87, 88 | `kwra_querverbindungen.py`, `NETZROLLEN`, Feld `rollen` (#4: „stark ausgehend“ und „stark eingehend“); `querverbindungen.py`, Feld `netzrollen`; `RiskInteractionSection.tsx`, `netzrollenText()` | trägt |
| A9 | Dargestellt werden die Querverbindungen zwischen den 13 Handlungsfeldern: ein- und ausgehende Wirkbeziehungen je Handlungsfeld mit ihrer Anzahl, die Verbindung von Handlungsfeld zu Handlungsfeld in der Stärke der Anzahl ihrer Querbezüge (Abbildung 8 mit Hinweistext). | 83 | `kwra_handlungsfeld_querbezuege.py`, `HANDLUNGSFELDER` und `ABLESUNG` (ausgewiesene Ablesung aus Abb. 8, Genauigkeit ±3 und ±5); `querverbindungen.py`, Feld `handlungsfelder`; `QuerverbindungenHandlungsfelder.tsx`, Rangfolge mit „rund N“ und Modellgrenze | trägt teilweise |
| A10 | Die Ergebnisse werden nach Clustern und Systembereichen eingeordnet: ausgehende Wirkungen vor allem bei natürlichen Systemen und Ressourcen (Cluster Wasser und Land), eingehende bei naturnutzenden Wirtschaftssystemen, Infrastrukturen und Gebäuden sowie Menschen und sozialen Systemen; mögliche Kaskadeneffekte. | 84–85, 88 | `kwra_querverbindungen.py`, `AUSSAGEN["einordnung_cluster"]`, `AUSSAGEN["kaskadeneffekte"]` und `SYSTEMBEREICH_MATRIX` (Tabelle 28, Kap. 7, S. 153); `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`, Abschnitte „Annahme der Auswertung und Einordnung“ und „Querverbindungen zwischen den Systembereichen“; `QuerverbindungenHandlungsfelder.tsx`, „Einordnung nach Systembereichen“ | trägt |
| A11 | Die Klimawirkungen werden „auf mögliche gegenseitige Wechselwirkungen und damit Rückkopplungsschleifen untersucht“: Gewässertemperatur ↔ Kühlwasser; „Bedarf an Kühlenergie“ ↔ „Stadtklima/Wärmeinseln“; der Kreislauf Hitzebelastung → Bedarf an Kühlenergie ↔ Stadtklima/Wärmeinseln → Hitzebelastung ist der einzige Rückkopplungskreislauf der Gesamtauswertung, mit ausdrücklichem Verweis auf die Verantwortung der Kommunen; schwächer: „Verschiebung von Arealen …“ ↔ „Vegetation in Siedlungen“. | 85–86 (Abb. 9) | `backend/app/data/kwra_rueckkopplungen.py`, `RUECKKOPPLUNGEN` (RK-1 bis RK-3, RK-2 mit Verweis auf die Kommunen); `querverbindungen.py`, Feld `rueckkopplungen` mit `im_katalog` je Knoten; `RueckkopplungKreislauf.tsx`, Kreislauf als kurze Liste | trägt |
| A12 | In einem weiteren Analyseschritt werden die Querverbindungen der als hoch bewerteten Klimawirkungen (Gegenwart, Mitte und Ende des Jahrhunderts) gesondert betrachtet (Ergebnis: „Biologische Vielfalt“ mit der höchsten Gesamtzahl; viele eingehende Beziehungen bei „Schäden an Küstenökosystemen“, „Schäden an Feuchtgebieten und wassergebundenen Habitaten“, „Schäden an Wäldern“, „Verschiebung von Arealen …“). | 86–87, 88 | `kwra_querverbindungen.py`, `HOCHRISIKO_BEFUNDE` und Feld `auswertungen` in `NETZROLLEN` (#4, #5 „gesamt“ und „hochrisiko“, #7 und #8 nur „hochrisiko“); `querverbindungen.py`, Felder `hochrisiko` und `netzrolle_auswertungen`; `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`, Abschnitt „Hoch bewertete Klimawirkungen“ | trägt |
| A13 | Die Ergebnisse werden mit der Querverbindungsauswertung der Vulnerabilitätsanalyse 2015 verglichen (ähnliche Muster, Kaskade Land/Wasser → Wirtschaft/Gesundheit/Infrastruktur, zentrale Rolle Wasserhaushalt). | 87 | keiner — nicht einschlägig, siehe Begründung | trägt nicht |

**Begründung je Urteil:**

- A1: Alle im Fließtext von Kap. 3.4 genannten Beziehungen stehen jetzt in `BENANNTE_BEZIEHUNGEN`, auch die in
  der ersten Gegenprobe fehlenden (S. 82, 85, 86); die Einträge, die nicht im Blatt „Wirkbeziehungen“ der
  Arbeitsmappe stehen, tragen den Vermerk „nicht aus der Arbeitsmappe“. Identifiziert sind damit aber 27 von
  257 Querbezügen, 15 davon zwischen einzelnen Klimawirkungen. Die übrigen lassen sich aus der Quelle nicht
  gewinnen, weil sie keine Kantenliste veröffentlicht; das Produkt weist das als Modellgrenze aus und
  modelliert die Kanten bewusst nicht selbst (Vorgabe P1). Die Anforderung ist deshalb nur zum Teil erfüllt.
- A2: Jede benannte Beziehung mit Ende auf einer einzelnen Klimawirkung ist gegen die Klimawirkungsketten
  (UBA 2016) abgeglichen, mit Seite und Urteil je Zeile; die Einträge auf Handlungsfeld- und gemischter Ebene
  sind begründet ausgenommen, weil sie kein einzelnes Ende haben. Für den Bestand, den das Produkt übernimmt,
  ist der Abgleich vollständig.
- A3: Richtung und gegenseitige Wechselwirkung sind als Werte des Feldes `richtung` getrennt; der Dienst zählt
  eine gegenseitige Beziehung bei beiden Klimawirkungen als aus- und eingehend.
- A4: Die Annahme steht wörtlich mit Seite im Datenmodul, in der Doku und im Dashboard.
- A5: Die Ergebnisse der KWRA sind übernommen und sichtbar: je Klimawirkung die Netzrolle, je Handlungsfeld die
  Rangfolge nach der Ablesung aus Abb. 8, dazu die Kennzahlen. Alle 13 Netzknoten außerhalb des Katalogs,
  darunter #92 und #49, erscheinen im Dashboard als benannte Knoten mit „nicht im Katalog“, ohne Index, Betrag
  oder Rang. Eine Anzahl ein- und ausgehender Wirkungen je Klimawirkung nennt die Quelle nicht; die Spalten
  „wirkt auf/beeinflusst von (benannt)“ zählen ausgewiesen nur die benannten Beziehungen.
- A6: Beide zentralen Klimawirkungen sind mit Seite gekennzeichnet; Hochwasser (#49) erscheint jetzt im Dashboard.
- A7: Alle vier Zahlen stimmen mit S. 83–84 überein und stehen jetzt auch im Dashboard.
- A8: Die Netzrolle ist mehrwertig; #4 ist Sender und Empfänger zugleich, die zweite Rolle mit Seite 87.
- A9: Die Anzahlen je Handlungsfeld sind als ausgewiesene Ablesung mit Vorgehen und Genauigkeit übernommen
  (Vorgabe P1). Die Verbindung von Handlungsfeld zu Handlungsfeld in ihrer Stärke (Dicke der Bänder in Abb. 8)
...
- A11: Alle drei Wechselwirkungen und der Kreislauf sind mit Seite geführt; der Kreislauf steht im Dashboard,
  „Bedarf an Kühlenergie“ als Knoten mit „nicht im Katalog“.
- A12: Die Auswertung der hoch bewerteten Klimawirkungen ist von der Gesamtbetrachtung getrennt (`auswertungen`
  je Netzrolle, `HOCHRISIKO_BEFUNDE`); Feuchtgebiete (#7) und Wälder (#8) sind als starke Empfänger geführt,
  mit Seite 87 und dem Vermerk „nicht aus der Arbeitsmappe“. Die Befunde stehen in Doku und Dienstantwort;
  das Dashboard zeigt die Herkunft einer Netzrolle nicht an.
- A13: Vergleich zweier Ausgaben der Bundesanalyse; für ein kommunales Werkzeug ohne Vorgängerauswertung nicht
  einschlägig und deshalb nicht in die Lücke der Zeile 9 übernommen. Für den Status der Zeile 9 zählen A1 bis A12.

Nicht als eigene Anforderung gewertet: die Ergebnisse und Kernaussagen der KWRA selbst (Rangfolgen,
Mittelwerte, Kasten S. 88); sie sind die Inhalte, an denen A5 bis A12 gemessen werden.

**Gelesene Seiten und Abschnitte:** Erste Gegenprobe (T-0741-ceo, 23.09.2026): Inventar mit
`python3 /opt/overlord/overlord/skripte/dokumente.py inventar` (172 Seiten); Inhaltsverzeichnis S. 6–7 ganz
überflogen; Kap. 3.4 „Analyse der Querverbindungen“ S. 82–88 vollständig im Text gelesen, einschließlich
Fußnoten 20 (S. 82) und 21 (S. 84) und des Kastens „Kernaussagen der Analyse der Querverbindungen“ (S. 88),
dazu S. 89 bis zum Beginn von Kap. 4 (Kapitelgrenze geprüft). Als Bild angesehen: S. 83 (Abbildung 8
„Querverbindungen zwischen den Handlungsfeldern“ mit Hinweistext) und S. 86 (Abbildung 9 „Kreislauf der
Querverbindungen zwischen den Klimawirkungen ‚Hitzebelastung‘, ‚Bedarf an Kühlenergie‘ und
‚Stadtklima/Wärmeinseln‘“). Kap. 3.4 enthält keine Tabelle. Fortschreibung (T-0946-cto, 25.09.2026): im
Produkt vollständig gelesen `backend/app/services/querverbindungen.py`,
`backend/app/data/kwra_handlungsfeld_querbezuege.py`, `docs/QUERVERBINDUNGEN_KLIMAWIRKUNGEN.md`,
`docs/QUERVERBINDUNGEN_ABGLEICH_UBA2016.md` (Abschnitte „Gegenstand“, „Quelle und Lesart“, „Ergebnis“);
`backend/app/data/kwra_querverbindungen.py`, `backend/app/data/kwra_rueckkopplungen.py` und die drei
Dashboard-Komponenten an den Stellen, die die Tabelle nennt. **Nicht gelesen:** in der Fortschreibung der
Normtext selbst (die Anforderungen sind unverändert übernommen, die Seitenangaben der neuen Belege haben die
Teilpakete gegen die Quelle geprüft); Kap. 3.1 und die Handlungsfeldkapitel der Teilberichte 2–5; Tabelle 28
(Kap. 7, S. 153) stützt nur den Beleg und wurde nicht erneut gelesen.

**Schluss:** Von 13 Anforderungen tragen jetzt zehn voll (A2 bis A8, A10 bis A12), zwei teilweise (A1, A9)
und eine nicht (A13, nicht einschlägig). Die Belege tragen die Anforderung von Kap. 3.4 damit weitgehend, aber
nicht ganz: Beide verbliebenen Lücken folgen daraus, dass die Quelle keine Kantenliste und keine Zahlen von
Handlungsfeld zu Handlungsfeld veröffentlicht. Zeile 9 bleibt deshalb `teilweise`; die Spalte „Lücke“ nennt nur
noch diese beiden Punkte. Die Zählungen in den Abschnitten „Nachtrag: Abschlusszählung“ und „Zusammenfassung“
zieht diese Fortschreibung nicht nach; das ist Sache der Gesamtzählung (T-0487).
